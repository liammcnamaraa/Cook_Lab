...
def validate_simplex_tree(simplex_tree):  #
    ...
    if not isinstance(simplex_tree, gudhi.SimplexTree):  #
        raise ValueError("Input must be a gudhi.SimplexTree object")  #
    return True  #
...
def geometric_realization_point_cloud(simplex_tree, vertex_coords, num_samples_per_simplex=100, min_simplex_dim=0):  #
    """
    computes the geometric realization of a simplex complex represented as a SimplexTree
    ...
    if not all_sampled_points:  #
        if min_simplex_dim == 0:  #
            zero_simplices = [s for s in simplices if len(s) == 1]  #
            if zero_simplices:  #
                zero_indices = [s[0] for s in zero_simplices]  #
                return vertex_coords[list(set(zero_indices)), :]  #
        return np.array([]).reshape(0, vertex_coords.shape[1])  #
...
def compute_gromov_hausdorff_distance(
    ...
    for i in range(n):  #
        for j in range(n):  #
            diff_matrix = np.abs(C1_padded[i, :] - C2_padded[j, :])  #
            cost_matrix[i, j] = np.max(diff_matrix)  #
...
def create_simplex_tree_from_points_and_simplices(points, simplices):  #
    """
    creation of SimplexTree from pts and simplices, returing tuple of SimplexTree and vertex_coordinates
    ...
    for simplex in simplices:  #
        simplex_tree.insert(simplex, filtration=0.0)  #
...
def main_example():
    ...
    complex_pre, _ = create_simplex_tree_from_points_and_simplices(vertex_coords_pre, simplices_pre)  #
    ...
    complex_post, _ = create_simplex_tree_from_points_and_simplices(vertex_coords_post, simplices_post)  #
    print(f"number of simplices: {complex_post.num_simplices()}")  #
    ...
    realization_post = geometric_realization_point_cloud(complex_post, 
                                                        ...
                                                        num_samples_per_simplex=num_samples_simplex,
                                                        min_simplex_dim=min_dim_realization)
    print(f"pre geom real: {realization_pre.shape[0]} points.")
    print(f"post geom real: {realization_post.shape[0]} points.")
    ...
    print(f"  GH distance (minkowski L^3): {gh_dist_minkowski_p3:.4f}")
    
    gh_dist_euclidean_2 = compute_gromov_hausdorff_distance(  #
        realization_pre, realization_post,  #
        metric1='euclidean', metric2='euclidean'  #
    )  #
    print(f"  difference: {abs(gh_dist_euclidean - gh_dist_euclidean_2):.10f}")  #
...
def additional_example():  #
    ...
    simplex_tree1 = alpha_complex1.create_simplex_tree(max_alpha_square=1.0)  #
    
    alpha_complex2 = gudhi.AlphaComplex(points=points2)  #
    simplex_tree2 = alpha_complex2.create_simplex_tree(max_alpha_square=1.0)  #
    
    print(f"alpha1 complex{simplex_tree1.num_simplices()} simplices")  #
    print(f"alpha2 complex {simplex_tree2.num_simplices()} simplices")  #
    
  
    realization1 = geometric_realization_point_cloud(simplex_tree1, points1, num_samples_per_simplex=10)  #
    realization2 = geometric_realization_point_cloud(simplex_tree2, points2, num_samples_per_simplex=10)  #
    
    print(f"Realization 1: {realization1.shape[0]} points")  #
    print(f"Realization 2: {realization2.shape[0]} points")  #
    

    hausdorff_dist, _, _ = compute_hausdorff_distance(realization1, realization2)  #
    gh_dist = compute_gromov_hausdorff_distance(realization1, realization2)  #
    
    print(f" # H distance: {hausdorff_dist:.4f}")  #
    print(f"GH distance: {gh_dist:.4f}")  #
    
    
    gh_dist2 = compute_gromov_hausdorff_distance(realization1, realization2)  #
